...
async function sendPushToUser(user) {
  if (
...
async function notifyPushParticipants(conversation, senderId) {
  const recipients = state.users.filter(
    (u) =>
      u.id !== senderId &&
      canReceiveConversation(conversation, u) &&
      !isUserOnline(u.id)
  );
  await Promise.all(recipients.map((u) => sendPushToUser(u)));
}
...
async function handleApi(req, res, pathname, searchParams = null) {
  ...
  if (
    ...
    try {
      ...
      await notifyPushParticipants(conversation, auth.user.id);
      ...
      if (
        ...
        await notifyPushParticipants(conversation, info.senderId);
      }
